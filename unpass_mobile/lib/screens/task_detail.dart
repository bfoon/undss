...
class _TaskDetailScreenState extends State<TaskDetailScreen> {
  ...
  Future<void> _decide(String action, {required String verb}) async {
    ...
    if (action == 'reject' || action == 'return') {
      ...
      if (comment.trim().isEmpty) return; // cancelled, or nothing typed
    } else {
    ...
    setState(() => _busy = true);
    try {
      final message = await Api.instance.decide(widget.token, action, comment: comment);
      _changed = true;
      if (!mounted) return;
      ...
      if (e.stale) _reload(); // somebody else got there first
    }
  }
      ...
      child: Scaffold(
        ...
        body: FutureBuilder<Map<String, dynamic>>(
          ...
          builder: (context, snap) {
            ...
            if (snap.hasError) {
              return FailureState(message: '${snap.error}', onRetry: _reload);
            }
            final t = snap.data!;
            final answers = (t['answers'] as List<dynamic>? ?? []);
            final canDecide = t['can_decide'] == true && t['status'] == 'pending';

            return Stack(
              children: [
                ListView(
                  padding: const EdgeInsets.fromLTRB(UnStyle.gap, UnStyle.gap, UnStyle.gap, 140),
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        gradient: const LinearGradient(
                          colors: [UnColors.lightBlue, Color(0xFFF3F9FE)],
                          begin: Alignment.topLeft,
                          end: Alignment.bottomRight,
                        ),
                        borderRadius: BorderRadius.circular(UnStyle.radius),
                        border: Border.all(color: const Color(0x33009EDB)),
                      ),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('${t['step']}'.toUpperCase(),
                              style: const TextStyle(
                                  fontSize: 11.5, fontWeight: FontWeight.w800,
                                  color: UnColors.darkBlue, letterSpacing: 0.8)),
                          const SizedBox(height: 6),
                          Text('${t['title']}',
                              style: const TextStyle(
                                  fontSize: 20, fontWeight: FontWeight.w700, color: UnColors.navy)),
                          const SizedBox(height: 8),
                          Wrap(spacing: 10, runSpacing: 6, children: [
                            Text('${t['reference']}',
                                style: const TextStyle(fontFamily: 'monospace', fontSize: 12, color: UnColors.muted)),
                            if ('${t['started_by']}'.isNotEmpty)
                              Text('started by ${t['started_by']}',
                                  style: const TextStyle(fontSize: 12, color: UnColors.muted)),
                            if (t['due'] != null) StatusPill('Due ${t['due']}', color: UnColors.amber),
                          ]),
                        ],
                      ),
                    ),
                    ...
                    if ('${t['instructions']}'.isNotEmpty)
                      SectionCard(
                        ...
                        child: Text('${t['instructions']}', style: const TextStyle(height: 1.4)),
                      ),
                    if ('${t['message']}'.isNotEmpty)
                      SectionCard(
                        title: 'MESSAGE FROM THE SENDER',
                        icon: Icons.chat_bubble_outline,
                        child: Text('${t['message']}', style: const TextStyle(height: 1.4)),
                      ),
                        ...
                        child: Column(
                          children: [
                            for (final a in answers)
                              DetailRow('${(a as Map)['label']}', '${a['value']}'),
                          ],
                        ),
                      ),
                    SectionCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          OutlinedButton.icon(
                            onPressed: () async {
                              final url = Uri.parse(Api.instance.webUrl('${t['document_url']}'));
                              if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
                                if (context.mounted) {
                                  showNote(context, "Couldn't open the document.", error: true);
                                }
                              }
                            },
                    ...
                    if (!canDecide)
                      Container(
                        ...
                        child: Row(children: [
                          const Icon(Icons.info_outline, color: UnColors.amber),
                          ...
                          : Column(
                              ...
                              children: [
                                FilledButton.icon(
                                  ...
                                  onPressed: () => _decide(
                                      t['kind'] == 'review' ? 'acknowledge' : 'approve',
                                      verb: t['kind'] == 'review' ? 'Acknowledge' : 'Approve'),
                                  icon: Icon(t['kind'] == 'review' ? Icons.done_all : Icons.check_circle_outline),
                                  label: Text(t['kind'] == 'review' ? "I've reviewed it" : 'Approve'),
                                ),
                                const SizedBox(height: 8),
                                Row(children: [
                                  if (t['can_return'] == true) ...[
                                    Expanded(
                                      child: OutlinedButton.icon(
                                        style: OutlinedButton.styleFrom(foregroundColor: UnColors.amber),
                                        onPressed: () => _decide('return', verb: 'Return for changes'),
                                  ...
                                  Expanded(
                                    child: OutlinedButton.icon(
                                      ...
                                      onPressed: () => _decide('reject', verb: 'Reject'),
